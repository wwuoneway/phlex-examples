// Header for MCParticle from nutools simplified for testing Phlex and FORM. Original header by Brian Rebel in 2012.  Simplification by Andrew Olivier and Peter van Gemmeren in 2025.
...
inline const TLorentzVector& simb::MCParticle::Position(const int i) const
{
  return ftrajectory.Position(i);
}
inline const TLorentzVector& simb::MCParticle::Momentum(const int i) const
{
...
inline const TLorentzVector& simb::MCParticle::EndPosition() const
{
...
inline double simb::MCParticle::P(const int i) const
{
  return std::sqrt(std::pow(Momentum(i).E(), 2.) - std::pow(fmass, 2.));
}
inline double simb::MCParticle::Pt(const int i) const
{
  return std::sqrt(std::pow(Momentum(i).Px(), 2.) + std::pow(Momentum(i).Py(), 2.));
}

inline double simb::MCParticle::Mass() const { return fmass; }
inline const TLorentzVector& simb::MCParticle::EndMomentum() const
{
...
inline void simb::MCParticle::SparsifyTrajectory(double margin, bool keep_second_to_last)
{
...
inline bool simb::MCParticle::operator<(const simb::MCParticle& other) const
{
