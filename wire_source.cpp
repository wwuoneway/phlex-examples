...
#include "gaus_hit_finder/copied_from_larsoft_minor_edits/Wire.h"
...
PHLEX_REGISTER_PROVIDERS(m, config)
{
  ...
  part_maker.provide("provide_wires", &wrapper<ArtReader<recob::Wire>>::phlex_callback).output_product(product_query{.creator = "provide_wires", .layer = "event", .suffix = "wires"});
}
