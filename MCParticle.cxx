// Implementation for MCParticle from nutools simplified for testing Phlex and FORM.  Original file by Brian Rebel in 2012.  Simplified file by Andrew Olivier and Peter van Gemmeren in 2025.
...
namespace simb {
  ...
  //----------------------------------------------------------------------------
  void MCParticle::SetEndProcess(std::string s) { fendprocess = s; }
  ...
  //----------------------------------------------------------------------------
  void MCParticle::SetGvtx(TLorentzVector v) { fGvtx = v; }
  ...
  std::ostream& operator<<(std::ostream& output, const MCParticle& particle)
  {
    ...
    output << ", mass=" << particle.Mass() << ", Mother ID=" << particle.Mother()
           << ", Process=" << particle.Process() << ", Status=" << particle.StatusCode()
           << "\nthere are " << particle.NumberTrajectoryPoints() << " trajectory points";

    if (particle.NumberTrajectoryPoints() > 0)
      output << "\nInitial vtx (x,y,z,t)=(" << particle.Vx() << "," << particle.Vy() << ","
             << particle.Vz() << "," << particle.T() << "),\n Initial mom (Px,Py,Pz,E)=("
